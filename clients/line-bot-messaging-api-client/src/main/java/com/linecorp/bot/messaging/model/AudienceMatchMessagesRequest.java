...
public record AudienceMatchMessagesRequest(
    /**
     * Destination of the message (A value obtained by hashing the telephone number, which is
     * another value normalized to E.164 format, with SHA256).
     */
  ...
  public static class Builder {
    ...
    public AudienceMatchMessagesRequest build() {
      ...
      if (!this.messages$set) {
        throw new IllegalStateException("'messages' must be set for AudienceMatchMessagesRequest.");
      }

      if (!this.to$set) {
        throw new IllegalStateException("'to' must be set for AudienceMatchMessagesRequest.");
      }

      return new AudienceMatchMessagesRequest(messages, to, notificationDisabled);
    }
  }
}
