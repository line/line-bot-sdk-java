...
public record IssueShortLivedChannelAccessTokenResponse(
    /**
     * A short-lived channel access token. Valid for 30 days. Note: Channel access tokens cannot be
     * refreshed.
     */
    ...
    public IssueShortLivedChannelAccessTokenResponse build() {
      ...
      if (!this.accessToken$set) {
        throw new IllegalStateException(
            "'accessToken' must be set for IssueShortLivedChannelAccessTokenResponse.");
      }

      if (!this.expiresIn$set) {
        throw new IllegalStateException(
            "'expiresIn' must be set for IssueShortLivedChannelAccessTokenResponse.");
      }

      if (!this.tokenType$set) {
        throw new IllegalStateException(
            "'tokenType' must be set for IssueShortLivedChannelAccessTokenResponse.");
      }

      return new IssueShortLivedChannelAccessTokenResponse(accessToken, expiresIn, tokenType);
    }
  }
}
