 ...
 * LINE Official Account leaves a group chat or multi-person chat.
 */
@JsonTypeName("leave")
  ...
  public static class Builder {
    ...
    public LeaveEvent build() {
      ...
      if (!this.timestamp$set) {
        throw new IllegalStateException("'timestamp' must be set for LeaveEvent.");
      }

      if (!this.mode$set) {
        throw new IllegalStateException("'mode' must be set for LeaveEvent.");
      }

      if (!this.webhookEventId$set) {
        throw new IllegalStateException("'webhookEventId' must be set for LeaveEvent.");
      }

      if (!this.deliveryContext$set) {
        throw new IllegalStateException("'deliveryContext' must be set for LeaveEvent.");
      }

      return new LeaveEvent(source, timestamp, mode, webhookEventId, deliveryContext);
    }
  }
}
