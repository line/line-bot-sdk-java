...
public record DeactivatedEvent(
  ...
  public static class Builder {
    ...
    public DeactivatedEvent build() {

      if (!this.timestamp$set) {
        throw new IllegalStateException("'timestamp' must be set for DeactivatedEvent.");
      }

      if (!this.mode$set) {
        throw new IllegalStateException("'mode' must be set for DeactivatedEvent.");
      }

      if (!this.webhookEventId$set) {
        throw new IllegalStateException("'webhookEventId' must be set for DeactivatedEvent.");
      }

      if (!this.deliveryContext$set) {
        throw new IllegalStateException("'deliveryContext' must be set for DeactivatedEvent.");
      }

      return new DeactivatedEvent(source, timestamp, mode, webhookEventId, deliveryContext);
    }
  }
}
