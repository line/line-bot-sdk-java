...
public record AccountLinkEvent(
    ...
    /**
     * Reply token used to send reply message to this event. This property won&#39;t be included if
     * linking the account has failed.
     */
  ...
  public static class Builder {
    ...
    public AccountLinkEvent build() {
      ...
      if (!this.timestamp$set) {
        throw new IllegalStateException("'timestamp' must be set for AccountLinkEvent.");
      }

      if (!this.mode$set) {
        throw new IllegalStateException("'mode' must be set for AccountLinkEvent.");
      }

      if (!this.webhookEventId$set) {
        throw new IllegalStateException("'webhookEventId' must be set for AccountLinkEvent.");
      }

      if (!this.deliveryContext$set) {
        throw new IllegalStateException("'deliveryContext' must be set for AccountLinkEvent.");
      }

      if (!this.link$set) {
        throw new IllegalStateException("'link' must be set for AccountLinkEvent.");
      }
