  ...
  public static class Builder {
    ...
    public JoinEvent build() {
      ...
      if (!this.timestamp$set) {
        throw new IllegalStateException("'timestamp' must be set for JoinEvent.");
      }

      if (!this.mode$set) {
        throw new IllegalStateException("'mode' must be set for JoinEvent.");
      }

      if (!this.webhookEventId$set) {
        throw new IllegalStateException("'webhookEventId' must be set for JoinEvent.");
      }

      if (!this.deliveryContext$set) {
        throw new IllegalStateException("'deliveryContext' must be set for JoinEvent.");
      }

      if (!this.replyToken$set) {
        throw new IllegalStateException("'replyToken' must be set for JoinEvent.");
      }

      return new JoinEvent(source, timestamp, mode, webhookEventId, deliveryContext, replyToken);
    }
  }
}
