...
public class GetStatisticsPerUnitResponseTest {
    ...
    public void testDeserialize() throws IOException {
        final InputStream resource = getClass().getClassLoader().getResourceAsStream(
                "response/GetStatisticsPerUnitResponse.json");
        assertThat(resource).isNotNull();
        final GetStatisticsPerUnitResponse response =
                OBJECT_MAPPER.readValue(resource, GetStatisticsPerUnitResponse.class);
        assertThat(response).isNotNull();

        final Overview overview = response.getOverview();
        ...
        final Message message = response.getMessages().get(0);
